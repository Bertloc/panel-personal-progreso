...
export class SavingsManager {
  ...
  protected save() {
    ...
    (id ? this.api.updateGoal(id, payload) : this.api.createGoal(payload)).pipe(finalize(() => this.saving.set(false))).subscribe({ next: () => { this.reset(); this.events.notifyMoneyChanged(); this.load(); }, error: () => this.error.set('No se pudo guardar la meta.') });
  }
